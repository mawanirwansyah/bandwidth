...
class M_pengunjung extends CI_Model{
	...
	function statistik_pengujung(){
        $query = $this->db->query("SELECT tbl_pengunjung.*,DATE_FORMAT(pengunjung_tanggal,'%d') AS tgl,COUNT(pengunjung_ip) AS jumlah FROM tbl_pengunjung WHERE MONTH(pengunjung_tanggal)=MONTH(CURDATE()) GROUP BY DATE(pengunjung_tanggal)");
